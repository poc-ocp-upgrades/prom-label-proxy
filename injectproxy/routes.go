...
func NewRoutes(handler http.Handler, label string) *routes {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &routes{handler: handler, label: label}
}
...
func (r *routes) query(w http.ResponseWriter, req *http.Request) {
	...
	err = SetRecursive(expr, []*labels.Matcher{{Name: r.label, Type: labels.MatchEqual, Value: labelValue}})
	if err != nil {
		return
	}
...
func (r *routes) federate(w http.ResponseWriter, req *http.Request) {
	...
	req.URL.Query().Del(r.label)
	matcher := &labels.Matcher{Name: r.label, Type: labels.MatchEqual, Value: labelValue}
